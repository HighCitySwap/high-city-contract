...
contract HCCBODPool is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 unLockTime; // Time when the user can unlock the HCC.
    }
    ...
    IERC20 public HCC;
    // The HCC ReawrdPool!
    address public HCCBODReawardPool;
    // Info of each pool.
    ...
    function setHCCBODReawardPool(address _HCCBODReawardPool) public onlyOwner{
        HCCBODReawardPool = _HCCBODReawardPool;
        emit SetHCCBODReawardPoolEvent(_HCCBODReawardPool);
    }
    ...
    function _userInfo(address _user) public view returns(uint256 total,uint256 reward,uint256 lp,uint256 lockTime){
        PoolInfo storage pool = _poolInfo;
        UserInfo storage user = userInfo[_user];
        ...
        if (user.amount > 0) {
            ...
            return (user.amount,pendingAmount,accHCCPerShare,user.unLockTime);
        }
        return (0,0,0,0);
    }
