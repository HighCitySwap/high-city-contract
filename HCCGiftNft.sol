...
pragma solidity ^0.8.0;
interface IHCCNFTHolderPool {
    function userPowerChange(uint256 _amount, address _user) external;
}
...
contract HCCGiftNft is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    ...
    address public nftHolderPool;
    mapping(address => uint256) public _userPowerMap;
    Counters.Counter private _tokenIdTracker;

    event PauseEvent(bool pause);
    event OtherPaymentAddressChange(address newOtherPaymentAddress);
    event HccAddressChange(address newHccAddress);
    ...
    modifier onlyExecutor() {
        require(executorAddress == _msgSender(), " caller is not the executorAddress");
        _;
    }
    ...
    function setNftHolderPool(address newNftHolderPool) public onlyExecutor {
        nftHolderPool = newNftHolderPool;
        emit NftHolderPoolChange(newNftHolderPool);
    }
    ...
    function notifyPowerChange(address _findAddress) public {
        if (nftHolderPool != address(0)){
            IHCCNFTHolderPool(nftHolderPool).userPowerChange(_userPowerMap[_findAddress], _findAddress);
        }
    }
    ...
    function safeTransferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransfer(from, to, tokenId, _data);
        _userPowerMap[from] = _userPowerMap[from].sub(1);
        _userPowerMap[to] = _userPowerMap[to].add(1);
        notifyPowerChange(msg.sender);
    }
