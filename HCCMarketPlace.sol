...
contract MarketplaceStorage {
    ...
    mapping(uint256 => NormalOrder) public normalOrderMap;
    mapping(uint256 => AuctionOrder) public auctionOrderMap;

    uint256 public tradeFee;
    uint256 public publicationFeeInWei;
    // bid order extension time of bid success (in second)
    ...
    // EVENTS
    event NormalOrderCreated(
        bytes32 id,
        uint256 indexed assetId,
        address indexed seller,
        uint256 price
    );
    event NormalOrderSuccessful(
        bytes32 id,
        uint256 indexed assetId,
        address indexed seller,
        uint256 price,
    ...
    event AuctionOrderCancelled(
        bytes32 id,
        uint256 indexed assetId,
        address indexed seller
    );
    event AuctionOrderBidSuccess(
        bytes32 id,
        uint256 indexed assetId,
        address indexed seller,
...
contract HCCMarketplace is Initializable, Ownable, Pausable, MarketplaceStorage, NativeMetaTransaction {
    ...
    */
    ...
    /**
      * @dev Cancel an already published normal order
    *  can only be canceled by seller or the contract owner
    * @param assetId - ID of the published NFT
    */
    function cancelNormalOrder(uint256 assetId) public whenNotPaused {
        _cancelNormalOrder(assetId);
    }
    ...
    */
    function safeExecuteNormalOrder(
        uint256 assetId,
        uint256 price,
    ...
    */
    ...
    internal
    {
        ...
        normalOrderMap[assetId] = NormalOrder({
            id : orderId,
            seller : assetOwner,
            price : priceInWei
        });
    ...
    * @param assetId - ID of the published NFT
    */
    function _cancelNormalOrder(uint256 assetId) internal returns (NormalOrder memory) {
        address sender = _msgSender();
        NormalOrder memory order = normalOrderMap[assetId];
        ...
        delete normalOrderMap[assetId];

        emit NormalOrderCancelled(
            orderId,
            assetId,
            orderSeller
        );
    ...
    * @param fingerprint - Verification info for the asset
    */
    function _executeNormalOrder(
        uint256 assetId,
        uint256 price,
        bytes memory fingerprint
    )
    internal returns (NormalOrder memory)
    {
        ...
        NormalOrder memory order = normalOrderMap[_assetId];
        ...
        bytes32 orderId = order.id;
        delete normalOrderMap[_assetId];
    ...
    */
    ...
    /**
      * @dev Cancel an already published auction order
    *  can only be canceled by seller or the contract owner
    * @param assetId - ID of the published NFT
    */
    function cancelAuctionOrder(uint256 assetId) public whenNotPaused {
        _cancelAuctionOrder(assetId);
    }
    ...
    * @param fingerprint - Verification info for the asset
    */
    function safeExecuteAuctionOrder(
        uint256 assetId,
        bytes memory fingerprint
    )
    public
    ...
    * @param assetId - ID of the published NFT
    */
    function _cancelAuctionOrder(uint256 assetId) internal returns (AuctionOrder memory) {
        address sender = _msgSender();
        AuctionOrder memory order = auctionOrderMap[assetId];

        require(order.id != 0, "Asset not published");
        require(order.seller == sender || sender == owner(), "Unauthorized user");
        require(order.bidder == address(0), "The order is on sale and cannot be cancelled!");
    ...
    * @param fingerprint - Verification info for the asset
    */
    function _executeAuctionOrder(
        uint256 assetId,
        bytes memory fingerprint
    )
    internal returns (AuctionOrder memory)
    {
        address _nftAddress = nftAddress;
        uint256 _assetId = assetId;
        ...
        if(seller != nftRegistry.ownerOf(_assetId)){
            _returnBidder(order.id, order.bidder, order.bid);
            delete auctionOrderMap[assetId];
            return order;
        }
        ...
        bytes32 orderId = order.id;
        delete auctionOrderMap[_assetId];

        _transferMoney(order.bid, address(this), seller);
    ...
    */
    function _returnBidder(bytes32 orderId, address bidder, uint256 bid) internal {
        ...
        emit AuctionOrderBidFailed(orderId, bidder, bid);
    }
