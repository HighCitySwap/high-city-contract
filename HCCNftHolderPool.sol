...
contract HCCNFTHolderPool is Ownable {
    using SafeMath for uint256;
    ...
    IERC20 public HCC;
    // The HCC TBscen!
    address public HCCNftAddress;
    // The HCC ReawrdPool!
    address public HCCNFTReawardPool;
    // Info of each pool.
    PoolInfo public _poolInfo;
    // Info of each user that stakes LP tBscens.
    mapping(address => UserInfo) public userInfo;
    // Control mining
    bool public paused = false;


    event UserFightPoint(address indexed user, uint256 value);
    event Withdraw(address indexed user, uint256 amount);

    constructor(
        IERC20 _HCC,
        address _HCCNftAddress,
        address _HCCNFTReawardPool
    ) {
        HCC = _HCC;
        HCCNftAddress = _HCCNftAddress;
    ...
    modifier onlyHCCNFT {
        require(
            _msgSender() == HCCNftAddress,
            "HCCFightPointPool: caller is not the HCC"
        );
        _;
    }
    ...
    function updatePool() public {
        ...
        uint256 poolReward = HCC.balanceOf(HCCNFTReawardPool);
        if (poolReward <= 0) {
            return;
        }
    ...
    function pendingHCC(address _user) public view returns (uint256){
        PoolInfo storage pool = _poolInfo;
    ...
    function userPowerChange(uint256 _amount, address _user) public onlyHCCNFT{
        UserInfo storage user = userInfo[_user];
        PoolInfo storage pool = _poolInfo;
        updatePool();
        if (user.amount > 0) {
        ...
        //Deduct old FightPoint
        pool.totalAmount = pool.totalAmount.sub(oldValue);
        //Set NewFightPoint
        user.amount = _amount;
        pool.totalAmount = pool.totalAmount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accHCCPerShare).div(1e12);
        emit UserFightPoint(_user, _amount);
    }

    function withdrawHCC(address _user) public {
        PoolInfo storage pool = _poolInfo;
        UserInfo storage user = userInfo[_user];
        updatePool();
        uint256 pendingAmount = user.amount.mul(pool.accHCCPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function safeHCCTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > HCCBal) {
            HCC.transfer(_to, HCCBal);
        } else {
            HCC.transfer(_to, _amount);
        }
    }
